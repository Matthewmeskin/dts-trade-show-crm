  ...
  return (
    <form action={formAction} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <Field label="Full name" htmlFor="full_name" error={err.full_name}>
        <input id="full_name" name="full_name" className={inputClass} placeholder="e.g. Jane Smith" />
      </Field>

      <Field label="Email" htmlFor="email" required error={err.email}>
        <input id="email" name="email" type="email" autoComplete="off" className={inputClass} placeholder="name@dtsone.com" />
      </Field>
      ...
      <Field label="Temporary password" htmlFor="password" required error={err.password} hint="At least 8 characters. They can change it later.">
        <input id="password" name="password" type="text" autoComplete="new-password" className={inputClass} placeholder="Set an initial password" />
      </Field>

      <Field label="Role" htmlFor="role" hint="Admins can manage users.">
        <select id="role" name="role" defaultValue="standard" className={inputClass}>
