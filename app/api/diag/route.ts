...
export async function GET() {
  ...
  // This runs unauthenticated, and CRM access is now membership in profiles,
  // so RLS returns an empty set to an anonymous caller: `exhibitors: 0` with
  // ok:true is the healthy answer here, not an empty database. What this check
  // is really for is the schema — point the deployment at a schema that has no
  // CRM tables and the error reads "Could not find the table 'public.exhibitors'",
  // which is exactly the misconfiguration it exists to catch.
